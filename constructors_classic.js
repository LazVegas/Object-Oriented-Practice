...
// ???????? *******************************************************************
function Dog(obj) {
     this.status = 'normal'                 // expect(sadie.status).toBe('normal'
     if(obj) {
        this.color = obj.color              // expect(sadie.color).toBe('black')
     }
     if (obj && obj.hasOwnProperty("hungry")) {  //#3 
...
// #1 "It should make Sadie happy when Mason pets her"
...
// properties go on objects
// methods go on prototypes
// method is an action (verb)
// property is a descriptor

// last one: checking a property just like we did with color.  hasOwnProperty - see if property exists
...
var sadie = new Dog({               // passing an object
  color: "black",
  hungry: false
});

var moonshine = new Dog({           // pasing an object
  color: "blue-red"
});

var atticus = new Dog();            // passing a function
